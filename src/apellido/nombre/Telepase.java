...
public class Telepase extends Cabina {
	 ...
	 * 
	 */
	public void pagarAutomatico(Vehiculo vehiculo) throws TagNoEncontradoException, SaldoInsuficienteError {
		if (vehiculo.getTag() != null) {
			Boolean verificarTag = verifcarExiste(vehiculo.getTag());
			if (verificarTag) {
	...
	private void pagarConTelePase(Vehiculo vehiculo) throws SaldoInsuficienteError, TagNoEncontradoException {
		Double saldoDisponible = vehiculo.getTag().getSaldo();
		Double tarifaActual = this.getTarifaAutoBus();

		if (tarifaActual <= saldoDisponible && tarifaActual != null) {
			vehiculo.getTag().setSaldo(saldoDisponible - tarifaActual);
		} else {
...
//Verifica que si existe el tag
	public Boolean verifcarExiste(Tag tag) throws TagNoEncontradoException {
		...
		throw new TagNoEncontradoException("Tag no encontrado");
	}
